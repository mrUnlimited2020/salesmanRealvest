...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $propertyInvest['total_invested_property'] = Invest::where('user_id', $user->id)->count();
        $propertyInvest['total_invest'] = Invest::where('user_id', $user->id)->sum('total_invest_amount');
        $propertyInvest['total_due_amount'] = Invest::where('user_id', $user->id)->sum('due_amount');
        $propertyInvest['total_profit'] = Invest::where('user_id', $user->id)->sum('total_profit');

        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalWithdrawals', 'totalTransaction', 'countries', 'propertyInvest'));
    }
