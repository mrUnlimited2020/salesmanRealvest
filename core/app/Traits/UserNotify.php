...
trait UserNotify
{
    public static function notifyToUser()
    {
        return [
            ...
            'investedUser'           => 'Invested Users',
            'nonInvestedUser'        => 'Non Invested Users',
            'nextInstallmentUser'    => 'Pay Next Installment Users',
            'nextProfitUser'         => 'Get Next Profit Users',
            'referralUser'           => 'Users With Referral',
            'nonReferralUser'        => 'Users Without Referral',
        ];
    }

    public function scopeReferralUser($query)
    {
        return $query->whereHas('referrer');
    }

    public function scopeNonReferralUser($query)
    {
        return $query->whereDoesntHave('referrer');
    }

    public function scopeNextProfitUser($query)
    {
        return $query->whereHas('invests', function($invests){
            $invests->where('next_profit_date', '>', now())->where('profit_status', Status::RUNNING);
        });
    }

    public function scopeNextInstallmentUser($query)
    {
        return $query->whereHas('invests', function($invests){
            $invests->whereHas('installments', function($installments){
                $installments->where('next_time', '>', now())->where('status', Status::INSTALLMENT_PENDING);
            });
        });
    }

    public function scopeInvestedUser($query)
    {
        return $query->whereHas('invests');
    }

    public function scopeNonInvestedUser($query)
    {
        return $query->whereDoesntHave('invests');
    }
