                ...
                <form action="{{ route('admin.system.update.upload') }}" enctype="multipart/form-data" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="purchase_code" value="{{ env('PURCHASECODE') }}"
...
@if (!$general->system_customized)
    @push('breadcrumb-plugins')
        @if (!extension_loaded('zip'))
            <span class="text--danger mx-3"><span class="fw-bold text--danger">@lang('PHP-zip')</span>
                @lang('Extension is required')</span>
            <button type="button" disabled class="btn btn-sm btn-outline--primary"><i
                    class="las la-upload"></i>@lang('Upload')</button>
        @else
            <button type="button" class="btn btn-sm btn-outline--primary" data-bs-toggle="modal"
                data-bs-target="#uploadUpdate"><i class="las la-upload"></i>@lang('Upload')</button>
        @endif
    @endpush
@endif
